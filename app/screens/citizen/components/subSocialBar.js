...
import { CitizenCommentList } from './';
...
export class SubSocialBar extends React.Component {
    ...
    componentDidMount() {
        this.socket.on('subcitizen::reacted', this.onLikeCountReceive);
        this.socket.on('subcitizen::commented', this.onCmtCountReceive);
    }

    componentWillUnmount() {
        this.socket.off('subcitizen::reacted', this.onLikeCountReceive);
        this.socket.off('subcitizen::commented', this.onCmtCountReceive);
    }
    ...
    onLike = () => {
        ...
        if (isLiked) {
            ...
            url = citizenUrl + '/subposts/' + feedId + '/unlike';
        } else {
            this.setState({ isLiked: true, likesCount: likesCount + 1 });
            url = citizenUrl + '/subposts/' + feedId + '/like';
        }
    ...
    onUnLike = () => {
        ...
        if (isDisliked) {
            ...
            url = citizenUrl + '/subposts/' + feedId + '/undislike';
        } else {
            this.setState({ isDisliked: true, unlikesCount: unlikesCount + 1 });
            url = citizenUrl + '/subposts/' + feedId + '/dislike';
        }
    ...
    async getComments() {
        ...
        const url = citizenUrl + '/subposts/' + feedId + '/comments';
    ...
    onShare = () => {
        ...
        let shareOptions = {
            title: "Awlam",
            url: baseUrl + '/web/citizens/' + feedId,
