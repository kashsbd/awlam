...
import { citizenUrl, baseUrl, postUrl, Google_Api_Key } from '../../../utils/globle';
...
export class SubCitizenItem extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            //different modals
    ...
    renderAvatars = (medias) => {
        let assets = medias.map(media => {
            if (media.contentType.startsWith('image/')) {
                ...
                let mediaPath = citizenUrl + '/media/' + media._id + '/' + image_name()
    ...
    renderMedia(feed) {
        ...
        if (firstMedia.contentType.startsWith("video/")) {
            ...
            const videoPath = citizenUrl + '/stream/' + firstMedia._id;
            return (
                <TouchableOpacity
                    activeOpacity={0.8}
                    onPress={() => {
            ...
            return (
                ...
                >
                    <View>
                        <AutoResizeImage
                            width={Dimensions.get('window').width}
                            source={{ uri: citizenUrl + '/media/' + firstMedia._id + '/' + image_name() }}
                        />
                        {
                            feed.media.length === 1 ?
                                <View />
    ...
    deletePost(feed) {
        ...
        const url = citizenUrl + '/' + feed._id + '/delete';
    ...
    showSubPosts(feed) {
        this.props.navigation.navigate('SubCitizenList', { cid: feed._id });
    }
    ...
    render() {
        ...
        if (mediaPaths) {
            paths = mediaPaths.map(path => {
                return { source: { uri: citizenUrl + '/media/' + path._id + '/' + image_name() } };
            })
        }
