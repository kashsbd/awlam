...
export class SocialBar extends React.Component {
    ...
    componentDidMount() {
        this.socket.on('citizen::reacted', this.onLikeCountReceive);
        this.socket.on('citizen::commented', this.onCmtCountReceive);
    }

    componentWillUnmount() {
        this.socket.off('citizen::reacted', this.onLikeCountReceive);
        this.socket.off('citizen::commented', this.onCmtCountReceive);
    }
    ...
    onLike = () => {
        ...
        if (isLiked) {
            ...
            url = citizenUrl + '/' + feedId + '/unlike';
        } else {
            this.setState({ isLiked: true, likesCount: likesCount + 1 });
            url = citizenUrl + '/' + feedId + '/like';
        }
    ...
    onUnLike = () => {
        ...
        if (isDisliked) {
            ...
            url = citizenUrl + '/' + feedId + '/undislike';
        } else {
            this.setState({ isDisliked: true, unlikesCount: unlikesCount + 1 });
            url = citizenUrl + '/' + feedId + '/dislike';
        }
    ...
    async getComments() {
        ...
        const url = citizenUrl + '/' + feedId + '/comments';
