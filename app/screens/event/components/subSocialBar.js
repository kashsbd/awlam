...
import { eventUrl, baseUrl } from '../../../utils/globle';
import LoggedUserCredentials from '../../../utils/modal/LoggedUserCredentials';
import { EventCommentList } from './';
...
export class SubSocialBar extends React.Component {
    ...
    componentDidMount() {
        this.socket.on('subevent::reacted', this.onLikeCountReceive);
        this.socket.on('subevent::commented', this.onCmtCountReceive);
    }

    componentWillUnmount() {
        this.socket.off('subevent::reacted', this.onLikeCountReceive);
        this.socket.off('subevent::commented', this.onCmtCountReceive);
    }
    ...
    onLike = () => {
        ...
        if (isLiked) {
            ...
            url = eventUrl + '/subposts/' + feedId + '/unlike';
        } else {
            this.setState({ isLiked: true, likesCount: likesCount + 1 });
            url = eventUrl + '/subposts/' + feedId + '/like';
        }
    ...
    onUnLike = () => {
        ...
        if (isDisliked) {
            ...
            url = eventUrl + '/subposts/' + feedId + '/undislike';
        } else {
            this.setState({ isDisliked: true, unlikesCount: unlikesCount + 1 });
            url = eventUrl + '/subposts/' + feedId + '/dislike';
        }
    ...
    async getComments() {
        ...
        const url = eventUrl + '/subposts/' + feedId + '/comments';
