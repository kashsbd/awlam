...
import { eventUrl, baseUrl, postUrl, Google_Api_Key } from '../../../utils/globle';
...
export class SubEventItem extends PureComponent {
    ...
    renderAvatars = (medias) => {
        let assets = medias.map(media => {
            if (media.contentType.startsWith('image/')) {
                ...
                let mediaPath = eventUrl + '/media/' + media._id + '/' + image_name()
    ...
    renderMedia(feed) {
        ...
        if (firstMedia.contentType.startsWith("video/")) {
            ...
            const videoPath = eventUrl + '/stream/' + firstMedia._id;
            return (
                <TouchableOpacity
                    activeOpacity={0.8}
                    onPress={() => {
            ...
            return (
                ...
                >
                    <View>
                        <AutoResizeImage
                            width={Dimensions.get('window').width}
                            source={{ uri: eventUrl + '/media/' + firstMedia._id + '/' + image_name() }}
                        />
                        {
                            feed.media.length === 1 ?
                                <View />
    ...
    deletePost(feed) {
        ...
        const url = eventUrl + '/' + feed._id + '/delete';
