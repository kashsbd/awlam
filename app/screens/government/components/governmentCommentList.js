...
import { governmentUrl } from '../../../utils/globle';
...
export class GovernmentCommentList extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            message: '',
    ...
    sendComment = () => {
        ...
        if (message.trim().length != 0) {
            ...
            fetch(governmentUrl + '/' + feedId + '/comments', config)
                .then(res => res.json())
    ...
    getPostReactions() {
        ...
        const path = governmentUrl + '/' + this.props.feedId + '/reactions';
    ...
    _renderEachComment = (eachCmt) => {
        ...
        return (
            <EachComment comment={cmt} postType='GOVERNMENT' />
        );
    };
