...
export class GovernmentItem extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            //different modals
    ...
    renderAvatars = (medias) => {
        let assets = medias.map(media => {
            if (media.contentType.startsWith('image/')) {
                ...
                let mediaPath = governmentUrl + '/media/' + media._id + '/' + image_name();
    ...
    renderMedia(feed) {
        ...
        if (firstMedia.contentType.startsWith("video/")) {
            const placeholderPath = governmentUrl + '/media/' + feed.media[0]._id + '/thumbnail';
            const videoPath = governmentUrl + '/stream/' + firstMedia._id;
            return (
                <TouchableOpacity
                    activeOpacity={0.8}
                    onPress={() => {
            ...
            return (
                ...
                >
                    <View>
                        <AutoResizeImage
                            width={Dimensions.get('window').width}
                            source={{ uri: governmentUrl + '/media/' + feed.media[0]._id + '/' + image_name() }}
                        />
                        {
                            feed.media.length === 1 ?
                                <View />
    ...
    deletePost(feed) {
        ...
        const url = governmentUrl + '/' + feed._id + '/delete';
    ...
    render() {
        ...
        if (mediaPaths) {
            paths = mediaPaths.map(path => {
                return { source: { uri: governmentUrl + '/media/' + path._id + '/' + image_name() } };
            })
        }
