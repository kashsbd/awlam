...
import { governmentUrl, baseUrl } from '../../../utils/globle';
import LoggedUserCredentials from '../../../utils/modal/LoggedUserCredentials';
import { GovernmentCommentList } from './';
...
export class SocialBar extends React.Component {
    ...
    componentDidMount() {
        this.socket.on('government::reacted', this.onLikeCountReceive);
        this.socket.on('government::commented', this.onCmtCountReceive);
    }

    componentWillUnmount() {
        this.socket.off('government::reacted', this.onLikeCountReceive);
        this.socket.off('government::commented', this.onCmtCountReceive);
    }
    ...
    onLike = () => {
        ...
        if (isLiked) {
            ...
            url = governmentUrl + '/' + feedId + '/unlike';
        } else {
            this.setState({ isLiked: true, likesCount: likesCount + 1 });
            url = governmentUrl + '/' + feedId + '/like';
        }
    ...
    onUnLike = () => {
        ...
        if (isDisliked) {
            ...
            url = governmentUrl + '/' + feedId + '/undislike';
        } else {
            this.setState({ isDisliked: true, unlikesCount: unlikesCount + 1 });
            url = governmentUrl + '/' + feedId + '/dislike';
        }
    ...
    async getComments() {
        ...
        const url = governmentUrl + '/' + feedId + '/comments';
    ...
    onShare = () => {
        ...
        let shareOptions = {
            title: "Awlam",
            url: baseUrl + '/web/governments/' + feedId,
