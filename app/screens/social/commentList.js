...
import { MentionBoxWrapper, MentionText } from "../../components";
...
import EachLiker from "./eachLiker";
...
export class CommentList extends PureComponent {
  ...
  onLikePressed = () => {
    this.setState({ likerModalVisible: true, likerLoading: true }, () =>
      ...
      <View style={{ flex: 1 }}>
        ...
        {likerLoading ? (
          ...
          >
            <ActivityIndicator
              size={"large"}
              style={{ alignSelf: "center", color: Color.backgroundColor }}
            />
          </View>
  ...
  _renderEachComment = (eachCmt) => {
    ...
    return <EachComment comment={cmt} postType="POST" />;
  };
  ...
  _onChangeText = (text) => {
   ...
   // this.text_input && this.text_input.focusInput(false);
