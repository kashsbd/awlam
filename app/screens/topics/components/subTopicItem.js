...
import { topicUrl, baseUrl, postUrl, Google_Api_Key } from '../../../utils/globle';
...
export class SubTopicItem extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            //different modals
    ...
    renderAvatars = (medias) => {
        let assets = medias.map(media => {
            if (media.contentType.startsWith('image/')) {
                ...
                let mediaPath = topicUrl + '/media/' + media._id + '/' + image_name()
    ...
    renderMedia(feed) {
        ...
        if (firstMedia.contentType.startsWith("video/")) {
            ...
            const videoPath = topicUrl + '/stream/' + firstMedia._id;
            return (
                <TouchableOpacity
                    activeOpacity={0.8}
                    onPress={() => {
    ...
    showSubPosts(feed) {
        this.props.navigation.navigate('SubTopicList', { cid: feed._id });
    }
