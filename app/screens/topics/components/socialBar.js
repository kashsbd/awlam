...
export class SocialBar extends React.Component {
    ...
    componentDidMount() {
        this.socket.on('topic::reacted', this.onLikeCountReceive);
        this.socket.on('topic::commented', this.onCmtCountReceive);
    }

    componentWillUnmount() {
        this.socket.off('topic::reacted', this.onLikeCountReceive);
        this.socket.off('topic::commented', this.onCmtCountReceive);
    }
    ...
    onLike = () => {
        ...
        if (isLiked) {
            ...
            url = topicUrl + '/' + feedId + '/unlike';
        } else {
            this.setState({ isLiked: true, likesCount: likesCount + 1 });
            url = topicUrl + '/' + feedId + '/like';
        }
    ...
    async getComments() {
        ...
        const url = topicUrl + '/' + feedId + '/comments';
